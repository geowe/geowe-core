...
/**
 * Write CSV from VectorFeature and read CSV to VectorFeature.
 * First line indicates the attribute names.
 * Last column must be WKT.
 * 
...
public class CSV {
	...
	public List<VectorFeature> read(String geoDataString) {
		...
		for (int i = 1; i < csvLines.length; i++) {
			...
			Geometry geom = Geometry.fromWKT((csvFeature[csvFeature.length - 1]));
			VectorFeature feature = new VectorFeature(geom);
